  ...
  digest = (raw) => {
    ...
    const data = {
      ...
      bomb: bomb ? {
        ...
        site: bomb.state === "planted" || bomb.state === "defused" || bomb.state === "defusing" || bomb.state === "planting" ? _CS2GSI.findSite(
          raw.map.name,
      ...
      phase_countdowns: {
        ...
        timeout_team: raw.phase_countdowns.phase === "timeout_ct" ? teamCT : raw.phase_countdowns.phase === "timeout_t" ? teamT : void 0
      },
      auth: raw.auth,
      map: {
        mode: raw.map.mode,
    ...
    if (last.bomb && data.bomb) {
      if (last.bomb.state === "planting" && data.bomb.state !== "planting" && data.bomb.state !== "planted" && data.bomb.state !== "defusing") {
